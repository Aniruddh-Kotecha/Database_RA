...
Page BufferManager::getMatrixPage(string matrixName, int pageIndex)
{
    ...
    if (this->inPoolMatrix(pageName))
        ...
        return this->insertIntoPoolMatrix(matrixName, pageIndex);
}
 ...
 * @return Page 
 */
Page BufferManager::getFromPoolMatrix(string pageName)
{
 ...
 * @return Page 
 */
Page BufferManager::insertIntoPoolMatrix(string matrixName, int pageIndex)
{
    logger.log("BufferManager::insertIntoPoolMatrix");
    Page page(matrixName, pageIndex, true);
    if (this->pages.size() >= BLOCK_COUNT)
        pages.pop_front();
    pages.push_back(page);
    return page;
...
void BufferManager::writePage(string tableName, int pageIndex, vector<vector<int>> rows, int rowCount)
{
    ...
    // cout << "hewr" <<"\n";
    // cout << rowCount << " " << page.getRowCount() << "\n";
    page.writePage();
}
...
void BufferManager::writeMatrixPage(string matrixName, int pageIndex, vector<vector<int>> rows, int rowCount)
{
    ...
    Page matrixPage(matrixName, pageIndex, rows, rowCount, true);
    for(auto it : matrixPage.getRows()) {
        for(auto it1: it) {
            logger.log(to_string(it1));
        }
    }
...
void BufferManager::writeExistingPage(Page &page)
{
    logger.log("BufferManager::writeExistingPage");
    pages.clear();
    page.writeMatrixPage();
}
void BufferManager::writeExistingTablePage(Page &page)
{
    logger.log("BufferManager::writeExistingTablePage");
    pages.clear();
    page.writePage();
}
