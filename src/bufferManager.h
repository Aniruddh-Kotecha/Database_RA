 ...
 * is indicated by . within this system we simulate blocks by
...
class BufferManager{
    ...
    Page getFromPoolMatrix(string pageName);
    Page insertIntoPoolMatrix(string matrixName, int pageIndex);

    public:
    
    BufferManager();
    Page getPage(string tableName, int pageIndex);
    Page getMatrixPage(string matrixName, int pageIndex);
    ...
    void writeExistingPage(Page &page);
    void writeExistingTablePage(Page &page);
